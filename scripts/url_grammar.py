...
def main():
    ...
    ap.add_argument('--skip', default='node_modules,.git,dist,.vercel,.astro'); ap.add_argument('--check', action='store_true'); ap.add_argument('--no-links', action='store_true'); ap.add_argument('--exclude', default='')
    a = ap.parse_args()
    ...
    pg = pages(); changed = 0; added = 0; bad = []
    ...
    print(f'url_grammar: {len(pg)} pages · {changed} rewritten ({added} canonicals added) · grammar {WWW}/<path>{"/" if a.slash=="always" else ""}' + (f' · sitemap → {a.sitemap_out} ({len(pg)} urls)' if a.sitemap_out else ''))
    return 0
